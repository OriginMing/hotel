  ...
  <div class="homestay" v-if="homestayform">
    ...
    <el-form :model="homestayform" ref="homestayform">
      ...
      <el-form-item>
        ...
        <rich-text :value="homestayform.sdetail" formfield="sdetail" @change-rich="setSontice" ></rich-text>
      </el-form-item>
    <el-form-item label="入住须知"></el-form-item>
      <el-form-item>
        <rich-text :value="homestayform.snotice" :menus="['head', 'bold','fontSize','fontName', 'underline', 'strikeThrough']" formfield="snotice" @change-rich="setSontice"></rich-text>
      </el-form-item>
      <el-form-item>
...
import {SUCCESS,URL,IMGURL} from '@/lib/base.js';
import loading from '@/loading/loading.vue'
import instance from '@/http/http.js'
import RichText from '@/components/rich/Rich.vue'
  ...
  methods:{
    initHomestay(sid){
      homestayRead(sid).then(res=>{
        console.log("编辑加载",res);
        if(res.data){
         
        
          this.homestayform=res.data[0];
          console.log(this.homestayform.sid)
          this.bannerArr=res.data[0].sbanner;
           console.log("sname");
      console.log(res.data[0].sbanner);
console.log(res.data[0].sbanner.split(/,/) );
         this.bannerFileList=res.data[0].sbanner.split(/,/).map(ele=>({name:res.data[0].sname,url:this.IMGURL+ele })) 
 console.log("---------")
 console.log(this.bannerFileList);
    ...
    handleSubmit(){
      let sid = this.homestayform.sid;
        instance.put("/api/stayhome/"+sid,this.homestayform).then(res=>{
          if(res.code===SUCCESS){
